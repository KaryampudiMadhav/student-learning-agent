...
export function HistoryPage() {
  const { history, setHistory, streak, setStreak } = useAppStore()
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        // Calculate stats from the fresh response data instead of stale store state.
        if (historyData.length > 0) {
          const scores = historyData.filter((h) => h.score !== undefined && h.score !== null).map((h) => h.score)
